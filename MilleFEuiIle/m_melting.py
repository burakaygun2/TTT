...
class Melting:
    ...
    def add_melt_tracers(self):
        """If the temperature in the domain is within a given range below the melting temperature :math:`(T > T_{\\rm melt} - dT_{\\rm melt\_thresh}`\ ,
        see ``T_melt`` and ``dT_melt_thresh`` in ``m_parameters.py``), 
        the number of tracers is checked. If the number of tracers is lower than 10, the tracers are added.

        :var T_melt: the melting temperature
        :var dT_melt_thresh: a threshold for adding or removing tracers when melting

        .. note::
        
            If melting is the only mechanism requiring tracers, the tracers will be used only in the part of the
            domain where :math:`T > T_{\\rm melt} - dT_{\\rm melt\_thresh}`\ , which saves computation time.
    ...
    def delete_melt_tracers(self):
        """If the temperature in the domain drops enough below the melting temperature :math:`(T < T_{\\rm melt} - dT_{\\rm melt\_thresh}`\ ,
        see ``T_melt`` and ``dT_melt_thresh`` in ``m_parameters.py``), 
        the tracers are removed.

        :var T_melt: melting temperature
        :var dT_melt_thresh: threshold for adding or removing tracers when melting
