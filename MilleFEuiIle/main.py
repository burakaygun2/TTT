...
def run_code():
    """Imports the modules and runs the main time loop.
    ...
    while ((termination_condition[0] == "time" and float(t) < t_end)
        ...
        if (MeshClass.moving_mesh == True):
            ...
            if (mesh_movement == "Lagrangian"):
                EqClass.u_mesh.assign(project(EqClass.v_k*EqClass.dt, ElemClass.vCG1))
                EqClass.v_mesh.assign(project(Expression(("0.0", "0.0"), degree = 1), ElemClass.vCG1))
                EqClass.h_bot.assign(project(EqClass.h_bot + EqClass.v_k[1]*EqClass.dt, ElemClass.sCG1))
