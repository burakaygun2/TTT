...
class SaveFiles:
    def __init__(self, MeshClass, ElemClass):
        ...
        for i in range(len(Paraview_Output_Ini)):
            ...
            if (Paraview_Output_Ini[i] == "conductivity"):
                self.Function_Dict[Paraview_Output_Ini[i]] = self.conductivity
        ...
        for i in range(len(Paraview_Output)):
            ...
            if (Paraview_Output[i] == "conductivity"):
                self.Function_Dict[Paraview_Output[i]] = self.conductivity
    ...
    def Save_HDF5(self, step_output, step, dt, t):
        """At dedicated time steps defined by ``output_frequency``, writes quantities defined in ``Paraview_Output`` into a HDF5 file.
        The files can be used for reloading and for plotting.
    ...
    def Load_HDF5(self, t, dt, name, *args):
        """When ``reload_HDF5 == True``, reloads the HDF5 file from ``data_" + reload_name + "/HDF5/data.h5``.
        ...
        i = 0
        for arg in args:
            dataset = "/" + name + "/vector_%d" % (reload_step)
        ...
        sline = (lines[reload_step + 1]).split("\t\t")
    ...
    def save_mesh(self):
        """Saves the mesh as ``data_" + self.name + "/HDF5/mesh.xdmf`` and the subdomains defined by :func:`m_mesh.MeshModule.define_boundaries` as ``data_" + self.name + "/HDF5/subdomains.pvd``, both readable by Paraview.
