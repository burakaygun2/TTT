...
class MeshModule:
    ...
    def move_mesh(self, u_mesh):
        """Moves the mesh by the displacement :math:`\\boldsymbol{u}_{mesh}`.
    ...
    def create_mesh(self):
        """Creates a rectangular mesh with a bottom left point at :math:`(x,y) = (0,0)` and a top right point at :math:`(x,y) = (` ``length``, ``height``:math:`)`.
...
def count_nodes(bound_mesh):
    """Counts the nodes at the top boundary for :func:`m_mesh.MeshModule.detect_oscillations`.
