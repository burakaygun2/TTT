...
class Equations:
    def __init__(self, MeshClass, ElemClass, TracersClass, MeltingClass, FilesClass, t):
        ...
        self.conductivity = ElemClass.conductivity
    ...
    def equation_heat(self):
        ...
        * :math:`\\sigma_{SB}` is the Stefan-Boltzmann constant (in :mod:`m_constants`\ )
        ...
        if (nonlinear_heat_equation == True):
            ...
            if (mesh_movement == "Lagrangian"):
                eq_energy =  (rho_s*cp(self.Temp, self.composition)*(self.Temp - self.Temp_k)/self.dt*self.Temp_
                            
                        + 0.5*(k(self.Temp_k, self.composition)*dot(nabla_grad(self.Temp), nabla_grad(self.Temp_)))

                        + 0.5*(k(self.Temp_k, self.composition)*dot(nabla_grad(self.Temp_k), nabla_grad(self.Temp_))))*dx
            ...
            if (mesh_movement == "Lagrangian"):
                eq_energy = (rho_s*cp(self.Temp_k, self.composition)*(self._Temp - self.Temp_k)/self.dt*self.Temp_
                        ...
                        + 0.5*(k(self.Temp_k, self.composition)*dot(nabla_grad(self._Temp), nabla_grad(self.Temp_)))
    ...
    def V_tidal(self, y):
        lat = 90.0*np.pi/180.0
        ecc = 0.05
        n = 2.0*np.pi/(3.55*86400.0)

        T_star = 1.0/6.0*(1.0 - 3.0*cos(lat)**2)
        T_1 = ecc/2.0*(1.0 - 3.0*cos(lat)**2)*cos(n*self.t)
        
        return 3.0*GR*MP*RS**2/(2.0*AA**3)*((RS - height + y)/RS)**2*(T_star + T_1)#*conditional(gt(self.t/(3.55*86400), 1.0), 1.0, self.t/(3.55*86400))
    ...
    def a_tidal(self):
        ...
        return -3.0*GR*MP*RS/(AA**3)*(T_star + T_1)
    ...
    def equation_Stokes(self):
        ...
        if (elasticity == False):
            self.eq_momentum = (self._p*div(self.v_)
                ...
                # - rho(self.Temp, self.composition, self.xm)*self.a_tidal()*dot(self.e_z, self.v_)
        ...
        if (BC_Stokes_problem[1][0] == "free_surface" or phase_transition == True): 
            ...
            # \
            # + rho_l*self.V_tidal(self.x[1])*dot(-self.normal, self.v_)*self.ds(2)
    ...
    def solve_Stokes_problem(self, step, FilesClass):
        ...
        while (Picard_iter < Picard_iter_max and v_error > Picard_iter_error):
            ...
            if (BC_Stokes_problem[0][0] == "free_surface" and BC_Stokes_problem[1][0] == "free_surface"):
                # and BC_Stokes_problem[2][0] == "free_slip" and BC_Stokes_problem[3][0] == "free_slip"):
