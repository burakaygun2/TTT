...
def melt_interpolation(mesh, tracers_in_cells, tracers, column, function):
    """Interpolates the melt fraction and the associated temperature correction from the tracers to discontinuous FEM functions :math:`\\chi_m` and :math:`\\Delta T`, respectively.
...
def stress_interpolation(mesh, tracers_in_cells, tracers, stress_tensor):
    """Interpolates the deviatoric stress tensor components :math:`\\sigma_{xx}` and :math:`\\sigma_{xy}` from tracers to a discontinuous FEM function.
